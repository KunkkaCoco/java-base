...
public class StringUtil {
	 ...
	 *         assertEquals("aabbbccccx", StringUtil.reverse("xccccbbbaa"));
	 * 		   assertEquals("试测^%6cbA数参", StringUtil.reverse("参数Abc6%^测试"));
	 * </pre>
	 * </code>
	 */
	 ...
	 * 用例: <code>
	 *    &lt;input type="text" name="Name" value="<%=StringUtil.filterForHTMLValue(sContent)%>"&gt;
	 * </code>
	 * 
	 * @param _sContent
	 *            指定的文本内容. 如果为null则返回"".
	 ...
	 * @param array
	 * @return
	 * @see StringUtil.toString(Object[] array)
	 */
	...
	public static String getStringByEncoding(String str, String fromEncoding, String toEncoding) {
		...
		if (StringUtil.isEmpty(fromEncoding)) {
			return str;
		}
		try {
	 ...
	 * @param maxLength
	 *            指定的长度.
	 * @see StringUtilTest#testAdjustLength()
	 */
	...
	public static Map<String, String> parseAsMap(String src, String propertyDelimiter, String fieldDelimiter) {
		...
		//
		String[] properties = StringUtil.split(src, propertyDelimiter);
	...
	public static String join(Map<String, ?> entities, String separator) {
		...
		for (String key : keys) {
			buf.append(key).append("=").append(StringUtil.avoidNull(entities.get(key).toString())).append(separator);
		}
		return buf.toString();
	}
