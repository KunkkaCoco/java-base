...
import com.jmxClient.server.SimpleStandardMBean;
...
public class Client {
	...
	public void doTest() throws Exception {
		...
		System.out.println("================== 获取 Domain 域名 ==================");
		for (int i = 0; i < domains.length; i++) {
			echo("\tDomain[" + i + "] = " + domains[i]);
		}

		// 连结相关资源
		String className = "com.bear.server.SimpleStandard";
		String domain = "DefaultDomain";

		// 初始化主要Bean
		ObjectName stdMBeanName = new ObjectName(domain + ":type=" + className + ",name=2");
		echo("\nCreate SimpleStandard MBean...");
		mbsc.createMBean(className, stdMBeanName, null, null);

		// 获取属性
		echo("\tState = " + mbsc.getAttribute(stdMBeanName, "State"));
		echo("\tbChanges = " + mbsc.getAttribute(stdMBeanName, "NbChanges"));

		// 设置属性
		Attribute stateAttribute = new Attribute("State", "new state<<<<<<<<<,");
		mbsc.setAttribute(stdMBeanName, stateAttribute);

		echo("\nState = " + mbsc.getAttribute(stdMBeanName, "State"));
		echo("\tNbChanges = " + mbsc.getAttribute(stdMBeanName, "NbChanges"));
